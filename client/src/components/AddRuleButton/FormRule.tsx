  ...
  const makeCommand = (form: {command: string, value: string}[]) => {
    ...
    for(let i=0; i< form.length; i++) {
      if(form[i]&&form[i].value!='') {
        if((['-j', '--to-source', '--random', '--persistent', '--to-destination', '--log-prefix', '--log-level', '--set-mark', '--ttl-set', '--set-tos', '--set-dscp', '--selctx', '--set-connmark', '--ecn-tcp-remove']).includes(form[i].command)) {
